...
export async function GET(request: Request): Promise<Response> {
  ...
  if (
    [...incoming.searchParams.keys()].some((key) => key !== "incident_id")
    || incoming.searchParams.getAll("incident_id").length !== 1
  ) {
  ...
  return proxyControlPlane({
    request,
    method: "GET",
    path: `/api/v1/console/incidents/${incidentId}/attack-trace`,
  });
}
