# AI-SOC V4.0 Rust-first 迁移状态

依据：`AI-SOC_Rust_AI-Web-Guard_项目计划书_V4.0.docx`。  
更新：2026-08-11 / Migration 06。
...
- `aisoc-ingest`：mTLS proxy identity 边界、heartbeat/event schema、batch 幂等、backpressure、raw append-only evidence。

### 分析数据面

`aisoc-ingest -> aisoc-normalize -> aisoc-detection -> aisoc-incident`

本地 pipeline journal 用于崩溃恢复；PostgreSQL 已通过 `aisoc-storage::central::CentralStore` 成为 base profile 的 central authoritative repository，保存 Agent inventory、batch/raw index、normalized event、Detection、Incident、watermark 和 DLQ 状态。
...
3. P3 JetStream transport、Object Store immutable body、late/gap reconciliation、stream/backpressure 故障注入未闭环。
4. P4-P12 中若干高级能力虽然已有 native crate/骨架或部分实现，但尚未完成与旧 Python 行为的完整 production cutover 和 Linux/PostgreSQL 验收。
5. P13 hardening 与 P14 pilot 未执行。

## 下一迁移顺序

1. 在可信 Rust 1.82 builder 重建并审查 `Cargo.lock`，运行完整 workspace CI。
2. 在真实 PostgreSQL 执行 central repository + DLQ replay integration，加入 DB restart/transaction/crash-window 故障注入。
3. P3 实现 Rust JetStream + Object Store production profile，关闭 replay/late/gap/backpressure 门禁。
4. 随后按 P4→P12 热路径继续做行为等价迁移，只有 Rust 侧验收完成后再删除对应 Python production baseline。
