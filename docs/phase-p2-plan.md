...
| 诊断 CLI | `aisoc-probe-platform` 输出 CapabilityReport v0.1.0 JSON | 已实现 |
...
- 单元测试使用可注入的虚拟 `/etc`、`/proc`、`/sys` 路径验证所有分支。
...
- Agent、Heartbeat、EventBatch 和 BatchAck v0.1.0 均已导出 Draft 2020-12 JSON Schema，
  `aisoc-export-schemas --check` 会真实检测缺失或漂移，并已加入 CI。
...
- 数据库迁移 head 为 `20260803_0003`，`alembic check` 无漂移。带真实 PostgreSQL 的当前完整
  当时本地门禁为 121 tests passed、8 个 POSIX/链接用例 skipped、总覆盖率 81%；Ruff、mypy、
...
- runtime 的每轮工作会显式驱动可选 `run_once` Collector；poll 异常与其他 Collector 隔离。
  `aisoc-agent run` 已注册 audit.log 文件 Collector，Heartbeat 会传播 degraded/failed 状态以及
...
- 上述 Collector 只在临时文件、可注入状态读取器和 SQLite 重启对照中动态验证；尚未在
...
- Linux 锁定环境以 UID 10001、只读源码挂载执行 runtime + queue 测试：18 passed；无权修改
  源码或本地 `.venv`。
- `aisoc-agent run --config` 现在是真实长运行入口：只读取私有、非链接且有界的 JSON 配置，
...
- 本地动态负例已覆盖启动/健康超时、崩溃、协议乱序、输出洪泛、argv 字面量和父环境不
  ...
  最终镜像为 `aisoc-security-platform:p2-supervisor-revalidation`，digest
...
- 本地动态负例覆盖 payload/签名/manifest/目标篡改、未授权 key/kind、重放、版本复用、
...
- `InstalledReleaseProcessHealthCheck` 从尚未激活的只读 candidate deployment 直接解析清单声明的
  `bin/aisoc-agent`，只调用固定 `health-probe`。Linux 动态升级对照证明健康候选才提交；健康
...
- `src/aisoc/ingest_gateway/`：独立 mTLS aiohttp 服务（`aisoc-ingest`），与 `api_server` 共享
...
- `src/aisoc/agent_core/transport.py`：同步 `httpx` mTLS 客户端，`post_heartbeat`/`post_batch`
  发送 `X-Agent-Session` 并回读续期后的 session value。
- `src/aisoc/agent_core/process.py`：`AgentProcessConfig` 新增可选 transport 配置（ingest_url +
...
- 测试：`tests/unit/test_transport.py`（mTLS 客户端 ssl/header/ack/错误，本地通过）；`tests/integration/test_ingest_mtls.py`
  ...
  `AISOC_TEST_DATABASE_URL`，CI/Docker 运行）。

### 仍未完成（P2 退出条件缺口）

- Linux 制品（自包含 tar.gz/DEB/RPM/systemd/离线依赖）与 `scripts/build_agent_artifact.py` 签名制品脚本。**tar.gz + Ed25519 签名脚本已建（`src/aisoc/agent_core/artifact_builder.py` + `scripts/build_agent_artifact.py`），产出可经 `ReleaseVerifier.verify` 通过；DEB/RPM/systemd/离线依赖仍待 VM 矩阵。**
...
- 自包含制品：`artifact_builder.py`（`build_payload_tar` PAX + 拒绝 symlink/特殊文件、`build_signed_artifact` + Ed25519）+ `scripts/build_agent_artifact.py` CLI；单测 `test_agent_artifact`（产出经 `ReleaseVerifier.verify` READY，symlink 拒绝，当时 POSIX 条件 skip）。
- 下载器 + pipeline：`downloader.py`（仅 HTTPS、精确 host allowlist、禁重定向/环境代理、
  不回显完整 URL、流式 64KiB、独立 payload 上限、超 expected_size 即中止、sha256/size 校验）+
  `installer_pipeline.py`（signed manifest preflight→download→payload verify→install）；单测覆盖
  未验证 manifest 零网络请求、happy、目标边界、redirect、
  URL secret、sha256/size/exceeds/http 错误、pipeline install + 篡改拒绝。Linux 网络故障注入待执行。
- compose + smoke：`deploy/compose/p2.yml`、`tests/smoke/linux_ingest_e2e_smoke.py`；smoke 本地 Docker PG 下跑通（见上）。
- 全门禁：ruff format/check、mypy（86 files）、schema check、pytest（136 passed/9 skipped，skip 全为当时未具备的 POSIX/链接条件）、pip-audit 无漏洞、SBOM 生成。
...
- 传输用 mTLS HTTPS + JSON 而非 gRPC：与现有 FastAPI/Pydantic 栈一致；serverAuth 叶证书由 CA 在启动时
  签发并写入私有临时目录。DNS 名使用 DNS SAN，字面 IP 使用 IP SAN；客户端同时启用 hostname 与
  `CERT_REQUIRED` 链验证，客户端和服务端显式设置 TLS 1.2 下限，客户端执行 HTTPS-origin URL 校验并
  禁用环境代理和重定向。
  监听接口 `ingest_host` 与证书身份 `ingest_server_name` 已拆分；通配监听必须显式配置精确 DNS/IP
  身份，避免把 `0.0.0.0`/`::` 写入不可用的服务端 SAN。
  API/Ingest 对 CA 私钥的启动加载统一为单文件描述符 `O_NOFOLLOW` 读取，并在该描述符上校验
  普通文件、单链接、私有权限和大小；Linux 并发替换故障注入仍未执行。
  真实中心域名、证书轮换/吊销和错误主机名对照仍需在 Linux VM/多主机环境验证。
