...
- `blue-team-agent run` 现在按显式 run-once 循环驱动 `AuditdCollector`，不会依赖隐藏线程。文件 tail
...
- 本轮没有启动 Docker，也没有把 Falco、auditd 或 eBPF 探针加载到宿主；AuditdCollector 只在
  Windows 临时普通文件和可注入 `auditctl` 状态上动态验证，不构成原生 Linux auditd 通过证据。
...
- 规则仍依赖完整回看窗口中的最新 exec generation 防 PID reuse；当真实 Collector 缺少 exec 时会
  ...
  fail closed，真实 PostgreSQL 跨 poll/重启行为留到 Kali 验证。

## 下一增量

1. 在 Kali 上用原生 auditd/auditctl 动态验证文件权限、EOE/超时、rotation/truncation、重启/换 boot、
...
5. 在 Kali/Ubuntu/Rocky 等真实 Linux VM 上验证 L1 降级、Falco/auditd 接入和至少三种发行版的 L2 能力；记录 drop、CPU/内存和已知限制。
