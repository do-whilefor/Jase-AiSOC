...
from aisoc.errors import AuthorizationError, EvidenceIntegrityError, NotFoundError
...
class LocalObjectStore:
    ...
    async def get(self, tenant_id: str, ref: str) -> bytes:
        ...
        self._safe_path(relative)  # defense-in-depth escape check
        try:
            data = await asyncio.to_thread(read_bytes_under_root, self._root, relative)
        except FileNotFoundError as error:
            raise NotFoundError("evidence", ref) from error
        except OSError as error:
            raise EvidenceIntegrityError(ref) from error
