...
from aisoc.domain.ai_review import AssuranceLevel
from aisoc.domain.detection import AttackState
from aisoc.domain.resources import Criticality, IncidentStatus
...
from aisoc.response_engine import LinuxCommandPlanner, build_response_plan
...
async def claim_next_response_action(
    ...
    *,
    ...
    record.lease_token_digest = sha256_hex(raw_token.encode())
...
def _verify_lease(
    ...
    *,
    ...
    digest = sha256_hex(lease.lease_token.encode())
    if (
        ...
        or not secure_compare(record.lease_token_digest, digest)
    ):
