...
from aisoc._rustcore import hmac_sha256_hex, sha256_hex
    ...
    async def deliver(
        ...
        *,
        ...
        headers = {
            ...
            "User-Agent": "aisoc-notification-worker/0.1",
            "X-AISOC-Webhook-Id": lease.notification_id,
            "X-AISOC-Webhook-Key-Id": self._key_id,
            "X-AISOC-Webhook-Timestamp": timestamp,
            "X-AISOC-Webhook-Signature": f"v1={signature}",
        }
        try:
...
def build_webhook_body(lease: NotificationLease) -> bytes:
    ...
    envelope = {
        ...
        "source": "urn:aisoc:response-control",
        "type": lease.topic,
