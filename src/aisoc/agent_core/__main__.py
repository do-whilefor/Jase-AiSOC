...
from aisoc.agent_core.process import (
    AgentProcessError,
    load_agent_process_config,
    run_agent_process,
)
from aisoc.agent_core.queue import LocalDiskQueue, QueueConfig
from aisoc.agent_core.runtime import AgentRuntime, RuntimeConfig
from aisoc.agent_core.supervisor import (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aisoc-agent")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"aisoc-agent failed: {error}\n")
        return 1
    except Exception:
        sys.stderr.write("aisoc-agent failed: runtime initialization error\n")
        return 1


def _run_health_probe(stop_event: threading.Event) -> int:
    with tempfile.TemporaryDirectory(prefix="aisoc-agent-health-") as temporary:
        root = Path(temporary)
