...
from aisoc.domain.identifiers import AgentId, AgentVersion, HostId, TenantId
from aisoc.platform import CapabilityReport
...
class AgentEnvelope(AgentContract):
    ...
    model_config = ConfigDict(
        json_schema_extra={"$id": "https://schemas.aisoc.local/agent-envelope/v0.1.0"}
    )
...
class AgentHeartbeat(AgentContract):
    model_config = ConfigDict(
        json_schema_extra={"$id": "https://schemas.aisoc.local/agent-heartbeat/v0.1.0"}
    )
...
class EventBatch(AgentContract):
    model_config = ConfigDict(
        json_schema_extra={"$id": "https://schemas.aisoc.local/event-batch/v0.1.0"}
    )
...
class BatchAck(AgentContract):
    model_config = ConfigDict(
        json_schema_extra={"$id": "https://schemas.aisoc.local/batch-ack/v0.1.0"}
    )
