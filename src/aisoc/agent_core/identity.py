...
from aisoc._rustcore import sha256_bytes, sha256_hex
from aisoc.domain.identifiers import is_valid_identifier
...
class AgentCertificateIdentity:
    ...
    def san_uris(self) -> tuple[str, ...]:
        return (
            "spiffe://aisoc.local/tenant/"
            f"{quote(self.tenant_id, safe='')}/agent/{quote(self.agent_id, safe='')}",
            f"urn:aisoc:host:{quote(self.host_id, safe='')}",
            f"urn:aisoc:installation:{quote(self.installation_id, safe='')}",
            f"urn:aisoc:hardware:{self.hardware_binding}",
        )
...
class LocalCertificateAuthority:
    ...
    def generate(
        cls,
        *,
        common_name: str = "AI-SOC Agent Development CA",
        now: datetime | None = None,
    ) -> Self:
    ...
    @classmethod
    def from_files(cls, private_key_path: Path, certificate_path: Path) -> Self:
        """Load a private CA key from one verified descriptor and its public certificate."""
        private_key_pem = _read_private_key_file(private_key_path)
        try:
            certificate_pem = certificate_path.read_bytes()
        except OSError as error:
            raise AgentIdentityError("the CA certificate file is unavailable") from error
        return cls.from_pem(private_key_pem, certificate_pem)
    ...
    def issue_server_certificate(
        ...
        *,
        ...
        if not normalized_hostname or lifetime <= timedelta(0) or lifetime > timedelta(days=90):
            raise AgentIdentityError("invalid TLS server certificate request")
        try:
            address = ip_address(normalized_hostname)
        except ValueError:
            try:
                dns_name = normalized_hostname.encode("idna").decode("ascii").lower()
            except UnicodeError as error:
                raise AgentIdentityError("invalid TLS server certificate hostname") from error
            ...
            if (
                ...
                raise AgentIdentityError("invalid TLS server certificate hostname")
            san_name = x509.DNSName(dns_name)
        else:
            if address.is_unspecified:
                raise AgentIdentityError("invalid TLS server certificate hostname")
            san_name = x509.IPAddress(address)
        self._require_valid_issuance_window(issued_at, lifetime)
        key = ec.generate_private_key(ec.SECP256R1())
        certificate = (
            x509.CertificateBuilder()
...
def rotation_challenge(old_certificate_pem: str, new_csr_pem: str) -> bytes:
    ...
    return (
        b"aisoc-agent-rotation-v1\x00"
        + certificate.fingerprint(hashes.SHA256())
        + sha256_bytes(csr.public_bytes(serialization.Encoding.DER))
    )
...
def _read_private_key_file(path: Path, *, max_bytes: int = 1024 * 1024) -> bytes:
    flags = (
        os.O_RDONLY
    ...
    try:
        ...
        raise AgentIdentityError("the CA private key file is unavailable") from error
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
            raise AgentIdentityError(
                "the CA private key must be a private single-link regular file"
            )
        if stat.S_IMODE(metadata.st_mode) & 0o077:
            raise AgentIdentityError(
                "the CA private key must not be accessible by group or other users"
            )
    ...
    if len(content) > max_bytes:
        raise AgentIdentityError("the CA private key exceeds the supported size")
    return content
