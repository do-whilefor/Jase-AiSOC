...
from aisoc._rustcore import secure_compare, sha256_bytes, sha256_hex
...
class ReleaseVerifier:
    ...
    def preflight(
        ...
        *,
        ...
        """Verify signed manifest policy before any payload bytes are fetched."""
        self._verify(release, None, state, now=now)
    ...
    def verify(
        ...
        *,
        ...
        return self._verify(release, payload, state, now=now)
    ...
    def _verify(
        ...
        *,
        ...
        if payload is not None:
            ...
            if not _constant_time_equal(sha256_hex(payload), manifest.payload_sha256):
...
def _selected_for_rollout(installation_id: str, rollout_id: str, percentage: int) -> bool:
    if percentage == 100:
        return True
    digest = sha256_bytes(f"{rollout_id}\0{installation_id}".encode())
...
def _constant_time_equal(left: str, right: str) -> bool:
    return secure_compare(left, right)
