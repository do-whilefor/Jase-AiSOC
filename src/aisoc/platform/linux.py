...
from aisoc._rustcore import probe_linux as rust_probe_linux
...
class LinuxProbePaths:
    ...
    apt_candidates: tuple[Path, ...] = (Path("/usr/bin/apt-get"), Path("/usr/bin/apt"))
    dnf_candidates: tuple[Path, ...] = (Path("/usr/bin/dnf"), Path("/bin/dnf"))
    yum_candidates: tuple[Path, ...] = (Path("/usr/bin/yum"), Path("/bin/yum"))
    zypper_candidates: tuple[Path, ...] = (Path("/usr/bin/zypper"),)
    pacman_candidates: tuple[Path, ...] = (Path("/usr/bin/pacman"),)
    apk_candidates: tuple[Path, ...] = (Path("/sbin/apk"), Path("/usr/sbin/apk"))
...
class LinuxPlatformAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        resolved_kernel = kernel_release or uname.release
        resolved_architecture = architecture or uname.machine
        self._native_report: dict[str, object] | None = None
        if paths is None:
            native = rust_probe_linux(resolved_kernel, resolved_architecture)
            if native is not None:
                self._native_report = native
                self._platform_info = PlatformInfo.model_validate(native["platform"])
                return
...
def _detect_platform(
    ...
    *,
    ...
    return PlatformInfo(
        ...
        package_manager=_package_manager(paths),
...
def _package_manager(paths: LinuxProbePaths) -> PackageManager:
    candidates = (
        (PackageManager.APT, paths.apt_candidates),
        (PackageManager.DNF, paths.dnf_candidates),
        (PackageManager.YUM, paths.yum_candidates),
        (PackageManager.ZYPPER, paths.zypper_candidates),
        (PackageManager.PACMAN, paths.pacman_candidates),
        (PackageManager.APK, paths.apk_candidates),
    )
    for manager, paths_for_manager in candidates:
        if any(_exists(path) for path in paths_for_manager):
            return manager
