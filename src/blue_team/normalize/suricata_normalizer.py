...
class SuricataNormalizer:
    ...
    def normalize(self, raw: RawInput) -> NormalizeResult:
        ...
        event_id = f"evt_suri{hashlib.sha256(canonical).hexdigest()[:16]}"
...
def _extensions_from_eve(eve_type: str, eve: dict[str, object]) -> dict[str, str | int]:
    """Extract HTTP/SSH extension fields the detection engine needs (§8.3).

    Extension keys follow the dotted ``http.*``/``ssh.*`` namespace so they pass
