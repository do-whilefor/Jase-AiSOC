"""Polling Suricata EVE JSON collector wired to the Agent's reliable queue.

The collector tails ``/var/log/suricata/eve.json`` (one JSON object per line),
normalizes each record with ``SuricataNormalizer``, and enqueues the canonical
``SecurityEvent``. It has no hidden thread: ``AgentRuntime.run_once`` drives
``run_once``, which atomically checkpoints the file cursor. A crash can replay
already queued records, but the stable Suricata ``timestamp`` + payload hash
dedupe key makes that at-least-once behavior deduplicable.
"""
...
from blue_team.agent_core.file_tail import (
    BoundedFileTail,
...
from blue_team.normalize.suricata_normalizer import SuricataNormalizer
from blue_team.platform import CollectorCapability, CollectorState

_MAX_STATE_BYTES = 4 * 1024 * 1024
_EVE_TIME = re.compile(r'"timestamp"\s*:\s*"([^"]+)"')


class SuricataCollectorError(RuntimeError):
    """The Suricata collector could not preserve its evidence guarantees."""
...
class _SuricataCollectorState(_StateModel):
...
class SuricataCollector:
    """Tail EVE JSON, normalize each record, and durably enqueue network facts."""

    def __init__(
        self,
        config: SuricataCollectorConfig,
        *,
        ...
        self._normalizer = SuricataNormalizer()
        self._started = False
        self._paused = False
    ...
    def start(self) -> None:
        if self._started:
            raise SuricataCollectorError("suricata collector is already started")
        ...
        try:
            ...
            raise SuricataCollectorError("suricata EVE log is unavailable") from error
        self._started = True
        self._save_state()

    def run_once(self) -> None:
        if not self._started:
            raise SuricataCollectorError("suricata collector is not started")
        if self._paused:
            return
        now = self._now()
        for line in self._source.poll(self.config.max_lines_per_poll):
            if line.error is not None:
                ...
                self._emit_diagnostic(
                    ...
                    detail=line.error,
                    event_time=_eve_time(line.message) or now,
                )
                continue
            ...
            raw = RawInput(
                ...
                raw_ref=f"agent://suricata/{self.config.boot_id}/{_line_digest(line.message)}",
            ...
            if result.event is None:
                ...
                self._emit_diagnostic(
                    ...
                    message=line.message,
                    detail=detail or "suricata normalization failed",
                    event_time=_eve_time(line.message) or now,
                )
                continue
    ...
    def capability(self) -> CollectorCapability:
        ...
        return CollectorCapability(
            ...
            incomplete_count=0,
            last_error=error,
            validated_version="suricata-eve-v0.1.0",
        )
    ...
    def _emit_diagnostic(
        ...
        *,
        ...
        event = SecurityEvent.model_validate(
            {
                "event_id": f"evt_surigap{digest[:16]}",
                "schema_version": "0.1.0",
                "event_type": "collector.suricata_gap",
                ...
                "outcome": "failure",
                "labels": {"suricata.gap_reason": reason},
                "extensions": {
                    "suricata.raw_line": message[:4096],
                    "collector.error": detail[:1024],
                },
                "raw_ref": f"agent://suricata-gap/{self.config.boot_id}/{digest[:24]}",
            }
        )
        self._enqueue(event, EventPriority.P1)

    def _save_state(self) -> None:
        state = _SuricataCollectorState(
            boot_id=self.config.boot_id,
    ...
    def _now(self) -> datetime:
        ...
        if value.tzinfo is None or value.utcoffset() is None:
            raise SuricataCollectorError("suricata collector clock must be timezone-aware")
        return value.astimezone(UTC)


def _eve_time(line: str) -> datetime | None:
    match = _EVE_TIME.search(line)
    if match is None:
        return None
    try:
...
def _load_state(path: Path) -> _SuricataCollectorState | None:
    if not os.path.lexists(path):
        return None
    try:
        metadata = path.lstat()
    except OSError as error:
        raise SuricataCollectorError("suricata collector state is unavailable") from error
    if (
        stat.S_ISLNK(metadata.st_mode)
        or not stat.S_ISREG(metadata.st_mode)
        or metadata.st_nlink != 1
    ):
        raise SuricataCollectorError("suricata collector state must be a private regular file")
    if metadata.st_size > _MAX_STATE_BYTES:
        raise SuricataCollectorError("suricata collector state exceeds its byte limit")
    if os.name != "nt" and stat.S_IMODE(metadata.st_mode) & 0o077:
        raise SuricataCollectorError("suricata collector state is accessible by group or other")
    try:
        return _SuricataCollectorState.model_validate_json(path.read_bytes())
    except (OSError, ValidationError) as error:
        raise SuricataCollectorError("suricata collector state is invalid") from error


def _save_state(path: Path, state: _SuricataCollectorState) -> None:
    ...
    try:
        ...
        raise SuricataCollectorError("suricata collector state could not be persisted") from error
    finally:
