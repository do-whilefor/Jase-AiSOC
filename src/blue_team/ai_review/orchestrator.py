...
from blue_team.ai_review.evidence_verifier import (
    blind_claims,
    detect_claim_conflicts,
    verify_claim_evidence,
)
from blue_team.ai_review.gate import AiReviewGate
from blue_team.ai_review.prompting import (
    PROMPT_VERSION,
...
from blue_team.ai_review.tool_gateway import ToolGateway, ToolGatewayError
...
from blue_team.domain.incident import IncidentEvidenceBundle
...
def _run_id(package: EvidencePackage, request: ModelRequest, provider: str) -> str:
    ...
    return f"mrun_{hashlib.sha256(material.encode()).hexdigest()[:24]}"


def _verifier_slot_id(package: EvidencePackage, position: int) -> str:
    material = f"{package.review_task_id}\0verifier\0{position}"
    return f"vslot_{hashlib.sha256(material.encode()).hexdigest()[:16]}"
