...
These pass whether or not the compiled ``blue_team_rust`` extension is built:
...
from blue_team import _rusthash
...
def test_sha256_hex_matches_hashlib(data: bytes) -> None:
    assert _rusthash.sha256_hex(data) == hashlib.sha256(data).hexdigest()


@pytest.mark.parametrize("data", _CASES, ids=lambda v: f"len{len(v)}")
def test_sha256_bytes_matches_hashlib(data: bytes) -> None:
    assert _rusthash.sha256_bytes(data) == hashlib.sha256(data).digest()
...
def test_hmac_sha256_hex_matches_hmac(key: bytes, msg: bytes) -> None:
    assert _rusthash.hmac_sha256_hex(key, msg) == hmac.new(key, msg, hashlib.sha256).hexdigest()
...
def test_rust_available_reflects_extension_import() -> None:
    ...
    ext_importable = importlib.util.find_spec("blue_team_rust") is not None
    assert _rusthash.rust_available() is ext_importable
...
def test_secure_compare_bytes(a: bytes, b: bytes, expected: bool) -> None:
    assert _rusthash.secure_compare(a, b) is expected


def test_secure_compare_str_matches_bytes() -> None:
    assert _rusthash.secure_compare("hello", "hello") is True
    assert _rusthash.secure_compare("hello", "world") is False
...
def test_secure_compare_matches_secrets() -> None:
    ...
    for a, b in [(b"abc", b"abc"), (b"abc", b"abd"), (b"", b"x")]:
        assert _rusthash.secure_compare(a, b) == secrets.compare_digest(a, b)
