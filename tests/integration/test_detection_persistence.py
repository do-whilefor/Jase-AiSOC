...
def _http_event(seq: int, src_ip: str, url: str, status: int, offset: int) -> SecurityEvent:
...
async def test_detection_persist_and_idempotent_replay(tmp_path: Path) -> None:
    ...
    events = [_http_event(i, "203.0.113.9", f"/p{i:03d}", 404, round(i * 0.1)) for i in range(301)]
    ...
    # list_detections filters by tenant and returns the one detection.
    async with database.session() as session:
        listed, total = await list_detections(
            session, tenant_id=TENANT, category="web.recon.scanning"
        )
