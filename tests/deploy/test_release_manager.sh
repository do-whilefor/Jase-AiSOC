...
make_release v1
make_release v2
AISOC_INSTALL_PREFIX="$TMP/install" AISOC_ENVIRONMENT=development bash "$MANAGER" install "$TMP/v1" v1
[[ "$(readlink -f "$TMP/install/current")" == "$TMP/install/releases/v1" ]]
AISOC_INSTALL_PREFIX="$TMP/install" AISOC_ENVIRONMENT=development bash "$MANAGER" install "$TMP/v2" v2
[[ "$(readlink -f "$TMP/install/current")" == "$TMP/install/releases/v2" ]]
[[ "$(readlink -f "$TMP/install/previous")" == "$TMP/install/releases/v1" ]]
AISOC_INSTALL_PREFIX="$TMP/install" AISOC_ENVIRONMENT=development bash "$MANAGER" rollback
[[ "$(readlink -f "$TMP/install/current")" == "$TMP/install/releases/v1" ]]
...
if AISOC_INSTALL_PREFIX="$TMP/tampered" AISOC_ENVIRONMENT=development \
  bash "$MANAGER" install "$TMP/v2" tampered >/dev/null 2>&1; then
...
bash "$ROOT/scripts/sign-rust-release.sh" "$TMP/signed" "$TMP/signing.pem" >/dev/null
AISOC_INSTALL_PREFIX="$TMP/signed-install" AISOC_ENVIRONMENT=production \
  AISOC_RELEASE_VERIFY_KEY="$TMP/verify.pem" bash "$MANAGER" install "$TMP/signed" signed

make_release unsigned
if AISOC_INSTALL_PREFIX="$TMP/unsigned-install" AISOC_ENVIRONMENT=production \
  AISOC_RELEASE_VERIFY_KEY="$TMP/verify.pem" bash "$MANAGER" install "$TMP/unsigned" unsigned >/dev/null 2>&1; then
